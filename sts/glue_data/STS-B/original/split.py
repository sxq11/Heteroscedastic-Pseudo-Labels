...
for set_name in ['train', 'dev']:
    with codecs.open(f'./{set_name}.tsv', 'r', 'utf-8') as data_fh:
        for _ in range(1):
...
for set_name in ['train', 'dev', 'test']:
    with open(f'./{set_name}_new.tsv', 'w') as f:
        f.write(titles)
        for i in range(len(new_contents_merged[set_name])):
            f.write(new_contents_merged[set_name][i])
print("STS-B-DIR dataset created! ('./glue_data/original/STS-B/(train_new/dev_new/test_new).tsv')")
