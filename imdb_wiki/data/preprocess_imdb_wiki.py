...
def visualize_dataset(db="imdb_wiki"):
    ...
    plt.savefig(os.path.join(img_folder, f"{db}_age_total.png"))
...
def make_balanced_testset(db="imdb_wiki", max_size=150, seed=666, verbose=True, vis=True, save=True):
    ...
    df.fillna({'SPLIT': 'train'}, inplace=True)
    if verbose:
        print(df)
    if save:
    ...
    if vis:
        ...
        if not os.path.exists(img_folder):
            os.makedirs(img_folder)
        plt.savefig(os.path.join(img_folder, f"{db}_age_distribution.png"))
        plt.show()
